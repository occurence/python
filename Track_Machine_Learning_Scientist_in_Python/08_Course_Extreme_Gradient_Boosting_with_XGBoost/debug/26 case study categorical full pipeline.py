...
kidney_data = pd.read_csv(r'D:\STUDY\python\Track_Machine_Learning_Scientist_in_Python\08_Course_Extreme_Gradient_Boosting_with_XGBoost\datasets\kidney.csv'
                          ,na_values='NaN')
# kidney_data['class'].replace({'ckd':0, 'notckd':1}, inplace=True)
# mappings = {'ckd': 0, 'notckd': 1}
# kidney_data['class'] = kidney_data['class'].replace(mappings)
kidney_data['class'] = kidney_data['class'].replace({'ckd': 0, 'notckd': 1})
# kidney_data['class'].replace({'ckd':0, 'notckd':1})
...
kidney_feature_names = ['age','bp','sg','al','su','bgr','bu','sc','sod','pot','hemo','pcv','wc','rbc','pc','pcc','ba','htn','dm','cad','appet','pe','ane','rc']
