...
df_kidney = pd.read_csv("https://assets.datacamp.com/production/repositories/943/datasets/82c231cd41f92325cf33b78aaa360824e6b599b9/chronic_kidney_disease.csv", names=kidney_cols,
                        na_values='?')

# Replace label values with 0 (ckd) and 1
# df_kidney['label'].replace({'ckd':0, 'notckd':1}, inplace=True)
df_kidney['label'] = df_kidney['label'].replace({'ckd': 0, 'notckd': 1})
...
transformers.extend([([num_col], [SimpleImputer(strategy='mean'),
...
transformers.extend([(cat_col, [SimpleImputer(strategy='most_frequent')]) for cat_col in
                    categorical_columns])
