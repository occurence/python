...
# print(times_scores.shape)
# print(scores.shape)
# print(scores_series)
# Bootstrap a rolling confidence interval for the mean score
# scores_lo = scores_series.rolling(20).aggregate(partial(bootstrap_interval, percentiles=2.5))
# scores_hi = scores_series.rolling(20).aggregate(partial(bootstrap_interval, percentiles=97.5))
# scores_lo = scores_series.rolling(20).apply(lambda x: bootstrap_interval(x)[0], raw=True)
# scores_hi = scores_series.rolling(20).apply(lambda x: bootstrap_interval(x)[1], raw=True)
...
# bootstrap_functions = [partial(bootstrap_interval, percentiles=percentile) for percentile in percentiles]
# scores_percentiles = scores_series.rolling(20).aggregate(bootstrap_functions)

# percentiles = [2.5, 97.5]
# bootstrap_functions = [partial(bootstrap_interval, percentiles=percentile) for percentile in percentiles]
# prices_perc_rolling = scores_series.rolling(20, min_periods=5, closed='right')
# scores_percentiles = prices_perc_rolling.aggregate(bootstrap_functions)
# Ensure bootstrap_interval receives a 2D array and correct percentiles
# scores_lo = scores_series.rolling(20, min_periods=5, closed='right').apply(
#     lambda x: bootstrap_interval(x.reshape(-1, 1), percentiles=[2.5, 97.5])[0], raw=True
# )

# scores_hi = scores_series.rolling(20, min_periods=5, closed='right').apply(
#     lambda x: bootstrap_interval(x.reshape(-1, 1), percentiles=[2.5, 97.5])[1], raw=True
# )
