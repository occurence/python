...
flights_bucket = onehot.fit(bucketed).transform(bucketed)
flights_bucket.select('depart', 'depart_bucket', 'depart_dummy').show(5)

flights_bucket.select('mon','dom','dow','carrier','flight','org','depart','duration','delay','km','org_idx','org_dummy','depart_bucket','depart_dummy').show(5, truncate=False)

assembler = VectorAssembler(inputCols=[
    'km','org_dummy','depart_dummy'
], outputCol='features')

flights_duration_assembled = assembler.transform(flights_bucket)

flights_duration_assembled.select('mon','dom','dow','carrier','flight','org','depart','duration','delay','km','org_idx','org_dummy','depart_bucket','depart_dummy','features').show(5, truncate=False)

from pyspark.ml.classification import LogisticRegression

flights_train, flights_test = flights_duration_assembled.randomSplit([0.8, 0.2], seed=43)
logistic = LogisticRegression().fit(flights_train)
...
# flights_duration_assembled['prediction'] = predictions
predictions.select('mon','dom','dow','carrier','flight','org','depart','duration','delay','km','org_idx','org_dummy','depart_bucket','depart_dummy','features','prediction').show(5, truncate=False)
...
dow_onehot = OneHotEncoder(inputCol="dow", outputCol="dow_dummy").fit(flights_bucket).transform(flights_bucket)
dow_onehot.select('mon','dom','dow','carrier','flight','org','depart','duration','delay','km','org_idx','org_dummy','depart_bucket','depart_dummy','dow_dummy').show(5, truncate=False)
mon_onehot = OneHotEncoder(inputCol="mon", outputCol="mon_dummy").fit(dow_onehot).transform(dow_onehot)
mon_onehot.select('mon','dom','dow','carrier','flight','org','depart','duration','delay','km','org_idx','org_dummy','depart_bucket','depart_dummy','dow_dummy','mon_dummy').show(5, truncate=False)

assembler = VectorAssembler(inputCols=[
    'km','org_dummy','depart_dummy','dow_dummy','mon_dummy'
], outputCol='features')

flights_mon_assembled = assembler.transform(mon_onehot)
flights_mon_assembled.select('mon','dom','dow','carrier','flight','org','depart','duration','delay','km','org_idx','org_dummy','depart_bucket','depart_dummy','dow_dummy','mon_dummy','features').show(50, truncate=False)
