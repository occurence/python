...
# Create an indexer
carrier_indexed = StringIndexer(inputCol='carrier', outputCol='carrier_idx').fit(flights_km).transform(flights_km)
carrier_encoded = OneHotEncoder(inputCols=['carrier_idx'], outputCols=['carrier_dummy'], dropLast=True)

# Indexer identifies categories in the data
carrier_encoded_model = carrier_encoded.fit(carrier_indexed)

# Indexer creates a new column with numeric index values
flights_onehot = carrier_encoded_model.transform(carrier_indexed)

# Repeat the process for the other categorical feature
org_indexed = StringIndexer(inputCol='org', outputCol='org_idx').fit(flights_onehot).transform(flights_onehot)
org_indexed.show(5)


# Create an instance of the one hot encoder
org_encoded = OneHotEncoder(inputCols=['org_idx'], outputCols=['org_dummy'], dropLast=True)

# Apply the one hot encoder to the flights data
org_encoded_model = org_encoded.fit(org_indexed)
flights_onehot = org_encoded_model.transform(org_indexed)
